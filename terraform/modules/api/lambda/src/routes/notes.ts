...
import { notes, notePages, noteMembers, pages, users } from '../schema';
...
app.post('/', authRequired, async (c) => {
  ...
  const result = await db
    ...
    .values({
      ...
      visibility: (body.visibility as 'private' | 'public' | 'unlisted' | 'restricted') ?? 'private',
      editPermission:
...
app.put('/:noteId/pages', authRequired, async (c) => {
  ...
  for (let i = 0; i < body.page_ids.length; i++) {
    await db
      ...
      .where(
        and(eq(notePages.noteId, noteId), eq(notePages.pageId, body.page_ids[i]!)),
      );
  }
...
app.get('/:noteId/members', authRequired, async (c) => {
  ...
  const result = await db
    ...
    .from(noteMembers)
    .where(
      and(eq(noteMembers.noteId, noteId), eq(noteMembers.isDeleted, false)),
    )
