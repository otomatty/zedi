...
describe('subscriptionService', () => {
  describe('getUserTier', () => {
    it('returns pro when active subscription exists', async () => {
      ...
      const tier = await getUserTier(userId, mockDb as never);
    ...
    it('returns cached value within TTL', async () => {
      ...
      const first = await getUserTier(userId, mockDb as never);
      expect(first).toBe('pro');

      vi.advanceTimersByTime(10_000);

      const second = await getUserTier(userId, mockDb as never);
  ...
  describe('getSubscription', () => {
    it('returns subscription when exists', async () => {
      ...
      const result = await getSubscription(TEST_USER_ID, mockDb as never);
