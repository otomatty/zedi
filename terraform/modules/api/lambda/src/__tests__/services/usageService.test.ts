...
describe('usageService', () => {
  describe('checkUsage', () => {
    it('returns allowed=true when within budget', async () => {
      ...
      const result = await checkUsage(TEST_USER_ID, 'free', mockDb as never);
    ...
    it('uses default budget when no tier budget found', async () => {
      mockDb.limit
        ...
        .mockResolvedValueOnce([{ totalCostUnits: 0 }]);

      const freeResult = await checkUsage(TEST_USER_ID, 'free', mockDb as never);
      ...
      const proResult = await checkUsage(TEST_USER_ID, 'pro', mockDb as never);
  ...
  describe('validateModelAccess', () => {
    it('returns model info for valid model', async () => {
      ...
      const result = await validateModelAccess('model-1', 'free', mockDb as never);
    ...
    it('throws for invalid or inactive model', async () => {
      ...
      await expect(
        validateModelAccess('nonexistent', 'free', mockDb as never),
      ).rejects.toThrow('Model not found or inactive');
    });
    ...
    it('throws FORBIDDEN when free user accesses pro model', async () => {
      ...
      await expect(
        validateModelAccess('model-pro', 'free', mockDb as never),
  ...
  describe('recordUsage', () => {
    it('inserts usage log and upserts monthly usage', async () => {
      mockDb.then.mockImplementationOnce(
        (r?: ((v: unknown) => unknown) | null) => Promise.resolve(undefined).then(r),
      );
