...
import {
  TEST_USER_ID, TEST_COGNITO_SUB, TEST_USER_EMAIL,
  createMockDb, type MockDb,
} from '../helpers/setup';
...
describe('Users API — authenticated flows', () => {
  ...
  describe('POST /api/users/upsert', () => {
    ...
    it('creates a new user when none exists', async () => {
      ...
      mockDb.returning.mockResolvedValueOnce([{
        id: TEST_USER_ID, cognitoSub: TEST_COGNITO_SUB,
        email: TEST_USER_EMAIL, displayName: 'Test User',
    ...
    it('updates existing user by cognito_sub', async () => {
      const existing = {
        id: TEST_USER_ID, cognitoSub: TEST_COGNITO_SUB,
        email: TEST_USER_EMAIL, displayName: 'Old Name',
      ...
      mockDb.returning.mockResolvedValueOnce([{
        ...existing, displayName: 'New Name', updatedAt: new Date(),
      }]);
    ...
    it('merges accounts by email when cognito_sub differs', async () => {
      const existingByEmail = {
        id: 'other-id', cognitoSub: 'old-sub',
        email: TEST_USER_EMAIL, displayName: 'Existing',
        avatarUrl: null, createdAt: new Date(), updatedAt: new Date(),
      };
      ...
      // Update cognito_sub
      mockDb.returning.mockResolvedValueOnce([{
        ...existingByEmail, cognitoSub: TEST_COGNITO_SUB,
      }]);
  ...
  describe('GET /api/users/:id', () => {
    it('returns user by ID', async () => {
      const user = {
        id: TEST_USER_ID, cognitoSub: TEST_COGNITO_SUB,
        email: TEST_USER_EMAIL, displayName: 'Test',
        avatarUrl: null, createdAt: new Date(), updatedAt: new Date(),
      };
