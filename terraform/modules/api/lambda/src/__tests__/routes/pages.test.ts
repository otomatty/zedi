...
import {
  TEST_USER_ID, OTHER_USER_ID, createMockDb, jsonRequest, type MockDb,
} from '../helpers/setup';
...
describe('Pages API — authenticated flows', () => {
  ...
  describe('POST /api/pages', () => {
    it('creates a new page and returns 201', async () => {
      ...
      mockDb.returning.mockResolvedValueOnce([{
        id: 'page-1', ownerId: TEST_USER_ID, sourcePageId: null,
        title: 'Test Page', contentPreview: null, thumbnailUrl: null,
        sourceUrl: null, createdAt: now, updatedAt: now, isDeleted: false,
      }]);
    ...
    it('creates a page with source_url for web clips', async () => {
      const now = new Date();
      mockDb.returning.mockResolvedValueOnce([{
        id: 'clip-1', ownerId: TEST_USER_ID, sourcePageId: null,
        title: 'Clipped', contentPreview: null, thumbnailUrl: null,
        sourceUrl: 'https://example.com', createdAt: now, updatedAt: now, isDeleted: false,
      }]);

      const res = await jsonRequest(app, 'POST', '/api/pages', {
        title: 'Clipped', source_url: 'https://example.com',
      });
    ...
    it('returns Y.Doc content as base64', async () => {
      ...
      mockDb.limit
        ...
        .mockResolvedValueOnce([{
          pageId: 'p1', ydocState: ydoc, version: 3,
          contentText: 'hello', updatedAt: new Date('2025-01-01'),
        }]);
  ...
  describe('PUT /api/pages/:id/content', () => {
    ...
    it('succeeds with optimistic lock when versions match', async () => {
      ...
      const res = await jsonRequest(app, 'PUT', '/api/pages/p1/content', {
        content: b64Content, expected_version: 3,
      });
    ...
    it('updates title alongside content', async () => {
      ...
      const res = await jsonRequest(app, 'PUT', '/api/pages/p1/content', {
        content: b64Content, title: 'Updated Title',
      });
