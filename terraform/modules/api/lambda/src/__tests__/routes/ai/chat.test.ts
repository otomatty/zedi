...
vi.mock('../../../env', () => ({
  getEnvConfig: vi.fn(() => ({
    CORS_ORIGIN: '*', MEDIA_BUCKET: 'b', AI_SECRETS_ARN: 'arn:aws:secretsmanager:test:ai',
...
describe('AI Chat API', () => {
  ...
  beforeEach(() => {
    ...
    mockCheckUsage.mockResolvedValue({
      allowed: true, budgetUnits: 10000, consumedUnits: 500,
      remaining: 9500, usagePercent: 5,
    });
  ...
  it('returns 429 when monthly budget is exceeded', async () => {
    mockCheckUsage.mockResolvedValueOnce({
      allowed: false, budgetUnits: 10000, consumedUnits: 10001,
      remaining: 0, usagePercent: 100.01,
    });
