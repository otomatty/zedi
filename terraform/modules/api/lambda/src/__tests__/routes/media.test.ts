...
vi.mock('../../env', () => ({
  getEnvConfig: vi.fn(() => ({
    CORS_ORIGIN: '*', MEDIA_BUCKET: 'test-media-bucket', AI_SECRETS_ARN: 'a',
    RATE_LIMIT_TABLE: 'r', THUMBNAIL_SECRETS_ARN: 'a', THUMBNAIL_BUCKET: 'b',
    THUMBNAIL_CLOUDFRONT_URL: 'https://t', ENVIRONMENT: 'test', POLAR_SECRET_ARN: 'a',
    COGNITO_USER_POOL_ID: 'p', COGNITO_REGION: 'us-east-1', AURORA_CLUSTER_ARN: 'a',
    DB_CREDENTIALS_SECRET: 'a', AURORA_DATABASE_NAME: 'zedi',
  })),
  resetEnvCache: vi.fn(),
}));
  ...
  describe('POST /api/media/confirm', () => {
    it('confirms upload and stores media record', async () => {
      ...
      mockDb.returning.mockResolvedValueOnce([{
        id: 'media-1', ownerId: TEST_USER_ID, s3Key: 'key',
        fileName: 'image.png', contentType: 'image/png',
        fileSize: 1024, pageId: null, createdAt: now,
      }]);

      const res = await jsonRequest(app, 'POST', '/api/media/confirm', {
        media_id: 'media-1', s3_key: 'key',
  ...
  describe('GET /api/media/:id', () => {
    it('redirects to signed S3 URL', async () => {
      mockDb.limit.mockResolvedValueOnce([{
        id: 'media-1', s3Key: 'users/test/media/1/image.png',
      }]);
