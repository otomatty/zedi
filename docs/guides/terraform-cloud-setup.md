...
- 同じ種類のトークンを GitHub Environment secret `TF_API_TOKEN` として登録する
- workflow 側では `TF_TOKEN_app_terraform_io=${{ secrets.TF_API_TOKEN }}` を渡す
- これにより `terraform/cloudflare/` の remote backend に対して CI から認証できる
