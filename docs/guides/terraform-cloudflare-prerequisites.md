...
**注意:** Terraform は「既存リソースを import する」前提のため、Cloudflare 側で**新規にリソースを作成する作業は不要**。既に `zedi-note.app` の DNS と Pages プロジェクト（`zedi`, `zedi-dev`, `zedi-admin`）があれば、トークンと Account ID を用意すればよい。
...
| Pages プロジェクト | `zedi`, `zedi-dev`, `zedi-admin`                                     | **Workers & Pages** → **Overview** |
