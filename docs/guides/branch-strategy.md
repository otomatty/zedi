...
バージョンは main ブランチで確定し、Release Please がバージョン更新・タグ・GitHub Release を自動化します。

1. **develop → main の PR を作成・マージ**

   `develop → main` の PR を GitHub 上で作成し、レビューを経てマージします。
   `git merge develop` を直接実行して push するのは PR レビューやステータスチェックを迂回するため行わない。
   詳細は [マージ方法のルール](#マージ方法のルール) を参照。

2. **main 上で Release Please が Release PR を作成**

   main への push（Conventional Commits）に応じて、Release Please が **main 向け**のリリース PR を自動作成します。`package.json` のバージョンと `CHANGELOG.md` が更新されます。

3. **必要に応じてバージョンを手動上書き（任意）**
...
GitHub の PR マージ時には **Squash and merge** と **Create a merge commit** のいずれかを選びます。**Rebase and merge** は履歴の追跡が難しくなるため、本プロジェクトでは使用しません。取り込む先ブランチと PR の種類に応じて、次のルールに従ってください。
...
**Create a merge commit** または **Squash and merge** のどちらでも構いません。リリース単位で 1 コミットにまとめたい場合は Squash でもよいです。重要なのは「main → develop」の同期 PR を **Squash しない** ことです。

> **注意**: main ブランチで **Require linear history** を有効にしている場合、Create a merge commit は使用できません。その場合は **Squash and merge** を選択してください。
...
4. **Require linear history**（オプション）
   - ✅ マージコミットを禁止し、**Squash のみ許可**（Rebase and merge は本プロジェクトでは使用しない）
   - この設定を有効にする場合、「develop → main」のリリース PR は **Squash and merge** を使用すること（[マージ方法のルール](#マージ方法のルール)を参照）
