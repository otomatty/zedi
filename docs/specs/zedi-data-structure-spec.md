...
**想定ユーザーフロー:** 共有ノートを開いた状態で「新規ページを追加」する操作（例: ノート内の「＋ ページを追加」ボタン、またはゴーストリンク `[[新規タイトル]]` をクリックして「このノートにページを作成」を選択）により、**そのノート専用の新規ページ** が作成される。このフローは想定する。

**論点:** その新規ページの `pages.owner_id` を誰にするか。

| 方針 | 内容 | メリット | デメリット・リスク |
|------|------|----------|---------------------|
| **A. 作成者をオーナー** | ページを追加したユーザー（作成者）を `owner_id` にする。 | 作成者が「自分のページ」一覧でもそのページを参照・編集できる。自分が書いたものを自分の資産として持てる。 | **社外秘・機密ノートで問題になる。** 例: 社外のメンバー（契約者）がノート内でページを作成すると、そのページのオーナーは契約者になる。契約終了後もページは契約者の「自分のページ」に残り得る。ノートオーナー（企業）がページの削除・移動を完全に制御しづらくなる。 |
| **B. ノートオーナーをオーナー** | ノートの所有者（`notes.owner_id`）を新規ページの `owner_id` にする。 | **機密性が保たれる。** ノートが社外秘なら、ノート内で誰がページを作成しても「ページの所有者はノートオーナー（社内）」のまま。作成者（メンバー）はノート経由でのみアクセスでき、「自分のページ」一覧には出ない。ノートオーナーがコンテンツの帰属を明確に保持できる。 | 作成者にとっては「自分が作ったページが自分の一覧に出ない」という挙動。ノートの文脈でしか見られない。 |
| **C. ノート設定で選択** | ノートごとに「ノート内で作成されたページのオーナー」を「ノートオーナー」か「作成者」で選べる。 | ユースケースに応じて切り替え可能。 | 実装・説明の複雑さ。設定のデフォルトをどうするか。 |

**推奨（機密性を優先）:**

- **B. ノートオーナーをオーナー** を採用する。
  - 共有ノート内で「新規ページを追加」して作成されたページは、**`pages.owner_id = notes.owner_id`**（そのノートのオーナー）とする。
  - `note_pages.added_by_user_id` には実際に追加操作を行ったユーザーを記録する（誰が作成したかは履歴として残る）。
  - これにより、社外秘・機密ノートであっても、ノート内で作成されたコンテンツの帰属はノートオーナーにあり、メンバーが離脱してもページはノートオーナーの管理下に残る。
- **「自分のページをノートに追加」** するフロー（既存ページをノートにリンクするだけ）は従来どおり、**そのページの `owner_id` は変更しない**（追加したユーザーがもともとオーナーならそのまま）。

**まとめ**

| 操作 | ページの owner_id |
|------|-------------------|
| 共有ノート内で「新規ページを追加」して作成 | **ノートのオーナー**（`notes.owner_id`） |
| 既存の「自分のページ」をノートに追加 | **変更しない**（元のページの `pages.owner_id` のまま） |
