...
クラウドDB間の同期が完了しても、端末側のローカルDB（IndexedDB）が古い状態のままの場合、
表示が更新されないことがあります。

この場合は **端末側のIndexedDBを削除** して、初回同期（フル同期）をやり直してください。

#### 端末側での対応（概要）

- **iOS Safari**: 設定 → Safari → 詳細 → Webサイトデータ → 該当ドメインを削除
- **Android Chrome**: 鍵アイコン → サイトの設定 → ストレージ → データを消去

※ IndexedDBのみの削除UIはないため、サイトデータ全体の削除になります。
