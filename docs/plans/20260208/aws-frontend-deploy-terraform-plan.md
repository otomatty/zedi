...
- **Terraform でデプロイしない理由:** ビルド成果物は環境依存（VITE\_\* の値で変わる）ため、Terraform の `null_resource` + `local-exec` でビルドするより、CI でビルド・アップロードする方が一般的で柔軟。
...
| 8   | **GitHub Actions: フロントデプロイ**      | ビルド → S3 sync → CloudFront invalidation。Secrets で VITE\_\* と AWS 認証情報を渡す                                                   | `.github/workflows/deploy-frontend.yml`  |
...
| 環境変数ガイド（本番 VITE\_\*） | `docs/guides/env-variables-guide.md`              |
...
3. タスク 8 の **deploy-frontend.yml** を追加し、本番用 VITE\_\* を Secrets に設定したうえで、main ブランチから初回デプロイを実行する。
