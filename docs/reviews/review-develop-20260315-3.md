# セルフレビュー: develop（未コミット変更） / Self-review: develop (uncommitted changes)

**日時 / Date**: 2026-03-15
**ベース / Base**: develop（作業ツリーの変更のみ） / develop (working tree only)
**変更ファイル数 / Files changed**: 19 files
**関連ファイル数 / Related files**: ~20 files
...
Refactor of Settings and Onboarding: General settings split into Profile/Display/Language/DataManagement/About cards; shared `ProfileFormFields` and `LanguageSelectField`; `SectionSaveStatus` for per-section save state; `SettingsOverview` navigation-only with `returnTo` links. Critical and Warning items addressed.
...
### 🔴 Critical（マージ前に修正必須） / Critical (must fix before merge)
...
### 🟡 Warning（修正を推奨） / Warning (recommended fixes)
...
### 🟢 Info（任意の改善提案） / Info (optional improvements)
...
## Lint / Format チェック / Lint and format checks
...
## Lint warning 対応方針（本 PR 以外の警告について） / Lint warning policy (outside this PR)
