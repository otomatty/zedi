...
- 現行スクリプトは「基準ブランチ（develop）向けのクローズ PR」のみ取得しているため、**別ブランチ向け**（例: fix/xxx, feature/xxx）にクローズされた PR のブランチは候補に入っていない。
- 上記をすべて削除対象にするには、クローズ済み未マージ PR の取得時に `--base` を付けず、リポジトリ全体の closed (unmerged) PR を対象にする必要がある。
