...
- **ai-api / thumbnail-api:**  
  ...
  - RLS を「ユーザーデータ用テーブルだけ」に限定し、subscriptions / ai_* には RLS を張らなければ、これらの Lambda は変更不要。  
...
| 項目 | 内容 |
|------|------|
| **対象テーブル** | users, pages, page_contents, notes, note_pages, note_members, links, ghost_links, media に RLS を導入。subscriptions / ai_* は現状のまま。 |
