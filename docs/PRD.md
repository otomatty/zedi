...
| **Local & Remote DB** | **Turso (libsql)**      | ローカルファーストアーキテクチャ。ローカルは `@libsql/client-wasm` による WASM SQLite + IndexedDB 永続化。クラウドは Turso。すべての読み書きはローカルで即座に処理（Rows Read = 0）し、認証ユーザーはバックグラウンドで60秒間隔の双方向同期。 |
...
| ✅   | 🔴 必須 | **ローカルファースト同期の実装**（WASM DB + バックグラウンド同期） |
| ✅   | 🟡 推奨 | オフライン → オンライン時のコンフリクト解決（updated_at 優先） |

**実装詳細:**
- **ローカルファースト:** すべての読み書きはローカル WASM データベースで即座に処理（Rows Read = 0）
- **バックグラウンド同期:** 認証済みユーザーのみ、60秒間隔で Turso クラウドと双方向同期
- **永続化:** IndexedDB によるブラウザリロード後のデータ維持
- **コンフリクト解決:** `updated_at` が新しい方を優先

**完了基準:** ✅ 認証済みユーザーのデータが Turso クラウドに保存され、複数デバイス間で同期される。ローカルファースト動作により通常操作での Rows Read = 0 を達成。
...
### 7.1 データベース同期アーキテクチャ（✅ ローカルファースト実装済み）
...
- **ローカル DB:** `@libsql/client-wasm` によるブラウザ内 SQLite (WASM)。IndexedDB に永続化
- **リモート DB:** Turso クラウドに接続。Clerk JWT トークンによる認証
- **同期方式:** バックグラウンドで60秒間隔の双方向同期。`updated_at` が新しい方を優先するコンフリクト解決
- **動作:** 完全なローカルファースト。すべての読み書きはローカルで即座に処理（Rows Read = 0）
- **オフライン対応:** オフラインでも読み書き可能。オンライン復帰後に自動同期
- **使用パッケージ:** `@libsql/client-wasm`, `vite-plugin-wasm`, `vite-plugin-top-level-await`
