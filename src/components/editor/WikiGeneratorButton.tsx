...
interface WikiGeneratorButtonProps {
  title: string;
  hasContent: boolean;
  /** インラインWiki生成のコールバック。Inline generation callback. */
  onGenerate: () => void;
  /** 現在の生成ステータス */
...
/**
 * Wiki 生成ボタン。タイトルがあり本文が未入力のときだけ表示し、
 * クリックで `onGenerate` を呼ぶ。
 *
 * Wiki generation button. Shows only when there is a title and no body
 * content; click invokes the inline `onGenerate` callback.
 */
export const WikiGeneratorButton: React.FC<WikiGeneratorButtonProps> = ({
  ...
  // タイトルがない場合は常に非表示。本文ありも非表示
  // (inline generation はページを上書きするため)。
  const hasTitle = title.trim() !== "";
  const shouldShowButton = hasTitle && !hasContent;
