...
describe("applyNoteEventToCache", () => {
  ...
  it("replaces the item in place on page.updated, keeping order", () => {
    ...
    const data = readCache(client);
    expect(data.pages[0]?.items.map((i) => i.id)).toEqual(["pg-1", "pg-2"]);
    expect(data.pages[0]?.items[1]?.title).toBe("Updated 2");
    expect(data.pages[0]?.items[1]?.content_preview).toBe("new preview");
  });

  it("is a no-op on page.updated when the id is not cached", () => {
    const client = seedCache([makeItem("pg-1")]);
    const event: NoteEvent = {
      type: "page.updated",
      note_id: NOTE_ID,
      page: makeItem("pg-missing"),
    };
    applyNoteEventToCache(client, event);

    const data = readCache(client);
    expect(data.pages[0]?.items.map((i) => i.id)).toEqual(["pg-1"]);
  });
  ...
  it("patches every window variant for the same note (different include/pageSize)", () => {
    ...
    const client = new QueryClient();
    const seedItems = [makeItem("pg-1")];
    const data: InfiniteData<NotePageWindowResponse, string | null> = {
      pages: [{ items: seedItems, next_cursor: null }],
      pageParams: [null],
    };
    client.setQueryData(
    ...
    client.setQueryData(noteKeys.pagesWindow(NOTE_ID, USER_ID, USER_EMAIL, ["preview"], 25), data);
