 ...
 * parent surface to close, so the callback is a stable module-scope no-op
 * (avoiding `useCallback` overhead for a function that never changes).
 *
 * 共有メニューはリンクタップ後に親 Sheet / Dropdown を閉じるための
 * `onClose` を受け取るが、ページ表示では閉じる対象がない。常に同じ no-op
 * を渡すだけなのでモジュールスコープに置き、`useCallback` のフック処理を
 * 走らせないようにする。
 */
const noopClose = () => {};
