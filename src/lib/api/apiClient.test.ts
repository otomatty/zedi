  ...
  describe('response unwrapping', () => {
    it('unwraps envelope response { ok: true, data: ... }', async () => {
      vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
        ...
        text: () => Promise.resolve(JSON.stringify({
          ok: true,
          data: { pages: [], links: [], ghost_links: [], server_time: '2025-01-01T00:00:00Z' },
        })),
        headers: new Headers(),
      }));
    ...
    it('accepts raw (non-envelope) response for backward compatibility', async () => {
      vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
        ...
        status: 200,
        text: () => Promise.resolve(JSON.stringify({
          pages: [], links: [], ghost_links: [], server_time: '2025-01-01T00:00:00Z',
        })),
        headers: new Headers(),
      }));
  ...
  describe('503 DATABASE_RESUMING auto-retry', () => {
    it('retries on 503 with Retry-After header', async () => {
      const fetchMock = vi.fn()
        .mockResolvedValueOnce({
          ok: false, status: 503,
          text: () => Promise.resolve(JSON.stringify({ error: 'Database is resuming', code: 'DATABASE_RESUMING' })),
    ...
    it('gives up after MAX_DB_RESUMING_RETRIES (4) attempts', async () => {
      ...
      const fetchMock = vi.fn().mockResolvedValue({
        ...
        text: () => Promise.resolve(JSON.stringify({
          message: 'Database is resuming', code: 'DATABASE_RESUMING',
        })),
        headers: new Headers({ 'Retry-After': '1' }),
      });
      vi.stubGlobal('fetch', fetchMock);
