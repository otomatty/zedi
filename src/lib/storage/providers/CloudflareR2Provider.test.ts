...
describe("CloudflareR2Provider", () => {
  ...
  it("throws on incomplete config", () => {
    expect(() => new CloudflareR2Provider({ ...VALID_CONFIG, bucket: "" })).toThrow(
      "Cloudflare R2 configuration is incomplete",
    );

    expect(() => new CloudflareR2Provider({ ...VALID_CONFIG, accountId: "" })).toThrow(
      "Cloudflare R2 configuration is incomplete",
    );

    expect(() => new CloudflareR2Provider({ ...VALID_CONFIG, accessKeyId: "" })).toThrow(
      "Cloudflare R2 configuration is incomplete",
    );

    expect(() => new CloudflareR2Provider({ ...VALID_CONFIG, secretAccessKey: "" })).toThrow(
      "Cloudflare R2 configuration is incomplete",
    );
  });
