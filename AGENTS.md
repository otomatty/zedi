...
- フック間およびフックから他ディレクトリへの import は相対パスではなく `@/` エイリアスを使う（サブディレクトリ移動時に壊れないため）。  
  _Import hooks (and other directories from hooks) via the `@/` alias rather than relative paths, so they survive moving between subdirectories._
