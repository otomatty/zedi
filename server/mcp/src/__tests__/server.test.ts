...
describe("createMcpServer", () => {
  ...
  it("zedi_delete_page calls client.deletePage and returns result", async () => {
    vi.mocked(mockClient.deletePage).mockResolvedValue({ id: "p1", deleted: true });
    const result = await mcpClient.callTool({
      name: "zedi_delete_page",
      arguments: { page_id: "p1" },
    });
    expect(result.isError).toBeFalsy();
    expect(mockClient.deletePage).toHaveBeenCalledWith("p1");
    const content = result.content as Array<{ text?: string }>;
    expect(content[0]?.text).toContain("deleted");
  });

  it("zedi_list_notes calls client.listNotes and returns JSON array", async () => {
    vi.mocked(mockClient.listNotes).mockResolvedValue([
      {
        id: "n1",
        owner_id: "u1",
        title: "My note",
    ...
    const result = await mcpClient.callTool({ name: "zedi_list_notes", arguments: {} });
    expect(result.isError).toBeFalsy();
    expect(mockClient.listNotes).toHaveBeenCalledOnce();
    const content = result.content as Array<{ text?: string }>;
    expect(content[0]?.text).toContain("n1");
  });

  it("zedi_get_note calls client.getNote with note_id", async () => {
    vi.mocked(mockClient.getNote).mockResolvedValue({ id: "n1", title: "N" });
    const result = await mcpClient.callTool({
      name: "zedi_get_note",
  ...
  it("zedi_create_note calls client.createNote and returns metadata", async () => {
    vi.mocked(mockClient.createNote).mockResolvedValue({
      id: "n2",
      owner_id: "u1",
      title: "New note",
      visibility: "private",
    ...
    expect(result.isError).toBeFalsy();
    expect(mockClient.createNote).toHaveBeenCalledWith({ title: "New note" });
    const content = result.content as Array<{ text?: string }>;
    expect(content[0]?.text).toContain("n2");
  });

  it("generic (non-ZediApiError) exceptions return isError with 'Unexpected error' label", async () => {
    vi.mocked(mockClient.listNotes).mockRejectedValue(new TypeError("unexpected boom"));
    const result = await mcpClient.callTool({ name: "zedi_list_notes", arguments: {} });
    expect(result.isError).toBe(true);
    const content = result.content as Array<{ text?: string }>;
  ...
  it("zedi_list_note_members calls client.listNoteMembers", async () => {
    vi.mocked(mockClient.listNoteMembers).mockResolvedValue([]);
    const result = await mcpClient.callTool({
      name: "zedi_list_note_members",
      arguments: { note_id: "n1" },
    });
    expect(result.isError).toBeFalsy();
    expect(mockClient.listNoteMembers).toHaveBeenCalledWith("n1");
  });
});