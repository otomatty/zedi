...
describe("loadMcpClientConfig", () => {
  ...
  it("ignores extra fields and returns only apiUrl and token", () => {
    ...
    const result = loadMcpClientConfig(file);
    expect(result).toEqual({ apiUrl: "https://api.example.com", token: "tok" });
    expect((result as unknown as Record<string, unknown>)?.someOtherField).toBeUndefined();
  });
  ...
  it("returns null when apiUrl is not a string", () => {
    const file = join(tmpDir, "badtype.json");
    writeFileSync(file, JSON.stringify({ apiUrl: 123, token: "tok" }), "utf-8");
    expect(loadMcpClientConfig(file)).toBeNull();
  });

  it("returns null when token is not a string", () => {
    const file = join(tmpDir, "notoken.json");
    writeFileSync(file, JSON.stringify({ apiUrl: "https://x", token: null }), "utf-8");
    expect(loadMcpClientConfig(file)).toBeNull();
  });
});