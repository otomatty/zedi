...
describe("HttpZediClient – additional edge cases", () => {
  let fetchMock: ReturnType<typeof vi.fn<typeof fetch>>;
  let client: HttpZediClient;
  ...
  it("omits query params with undefined values from the URL", async () => {
    fetchMock.mockResolvedValueOnce(makeJsonResponse(200, { results: [] }));
    // search with default scope and limit uses defined values; but if limit were
    // undefined it should be omitted. Test via search with a custom undefined-containing call.
    await client.search("test", "own", 10);
    const url = callArgs(fetchMock as unknown as Mock)[0] as string;
    expect(url).toContain("q=test");
    expect(url).toContain("scope=own");
    expect(url).toContain("limit=10");
    // No stray undefined= entries
    expect(url).not.toContain("undefined");
  });

  it("handles empty response body without throwing", async () => {
    // Some endpoints (like DELETE) may return 204 with no body.
    fetchMock.mockResolvedValueOnce(new Response("", { status: 200 }));
    const result = await client.deletePage("p1");
    // parsed will be undefined; returned as-is
    expect(result).toBeUndefined();
  });

  it("throws ZediApiError when 4xx body has no message field, using fallback text", async () => {
    fetchMock.mockResolvedValueOnce(makeJsonResponse(403, { code: "FORBIDDEN" }));
    await expect(client.getCurrentUser()).rejects.toMatchObject({
      name: "ZediApiError",
      status: 403,
      // No `message` field in body → falls back to "HTTP 403"
      message: "HTTP 403",
    });
  });

  it("throws ZediApiError when non-Error is thrown by fetch (string error)", async () => {
    fetchMock.mockRejectedValueOnce("network failure string");
    await expect(client.getCurrentUser()).rejects.toMatchObject({
      name: "ZediApiError",
      status: 0,
  ...
  it("getNote GETs /api/notes/:noteId", async () => {
    fetchMock.mockResolvedValueOnce(makeJsonResponse(200, { id: "n1", title: "My note" }));
  ...
  it("listNotePages GETs /api/notes/:noteId/pages", async () => {
    fetchMock.mockResolvedValueOnce(makeJsonResponse(200, []));
    await client.listNotePages("n1");
    const [url, init] = callArgs(fetchMock as unknown as Mock);
    expect(url).toBe(`${BASE}/api/notes/n1/pages`);
    expect((init as RequestInit).method).toBe("GET");
  });

  it("search returns empty array when results key is missing from response", async () => {
    fetchMock.mockResolvedValueOnce(makeJsonResponse(200, {}));
    const results = await client.search("q");
    expect(results).toEqual([]);
  });

  it("URL-encodes path segments with special characters (spaces, slashes)", async () => {
    fetchMock.mockResolvedValueOnce(makeJsonResponse(200, {}));
    await client.getNote("note with spaces/and/slashes");
    const [url] = callArgs(fetchMock as unknown as Mock);
    expect(url as string).toContain(encodeURIComponent("note with spaces/and/slashes"));
