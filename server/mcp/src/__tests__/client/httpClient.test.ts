...
describe("HttpZediClient error normalization", () => {
  ...
  it("exposes ZediApiError body for debugging on 4xx", async () => {
    const errorBody = { message: "note not found", code: "NOT_FOUND" };
    fetchMock.mockResolvedValueOnce(makeJsonResponse(404, errorBody));
    let caught: unknown;
    try {
      await client.getCurrentUser();
    } catch (e) {
    ...
    expect(caught).toBeInstanceOf(ZediApiError);
    expect((caught as ZediApiError).body).toEqual(errorBody);
  });
});

describe("HttpZediClient request shaping — additional methods", () => {
  let fetchMock: ReturnType<typeof vi.fn<typeof fetch>>;
  let client: HttpZediClient;
  ...
  it("getNote sends GET to /api/notes/:id", async () => {
    fetchMock.mockResolvedValueOnce(makeJsonResponse(200, { id: "n1" }));
  ...
  it("listNotePages sends GET to /api/notes/:noteId/pages", async () => {
    fetchMock.mockResolvedValueOnce(makeJsonResponse(200, []));
    await client.listNotePages("n1");
    expect(callArgs(fetchMock as unknown as Mock)[0]).toBe(`${BASE}/api/notes/n1/pages`);
    expect((callArgs(fetchMock as unknown as Mock)[1] as RequestInit).method).toBe("GET");
  });

  it("search uses default scope=own and limit=20 when omitted", async () => {
    fetchMock.mockResolvedValueOnce(makeJsonResponse(200, { results: [] }));
  ...
  it("search returns empty array when results field is missing from response", async () => {
    fetchMock.mockResolvedValueOnce(makeJsonResponse(200, {}));
    const results = await client.search("missing");
    expect(results).toEqual([]);
  });

  it("encodes special characters in page IDs in URL path", async () => {
    fetchMock.mockResolvedValueOnce(makeJsonResponse(200, { id: "p/1", deleted: true }));
