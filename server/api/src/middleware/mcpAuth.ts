 ...
 * `kv` を渡した場合は deny-list を参照し、失効済みトークンを拒否する。
 *
 * Verifies the Bearer token from request headers; throws HTTPException 401 on failure.
 * When `kv` is provided, revoked tokens are rejected via the deny-list.
 */
async function extractAndVerify(
  ...
  try {
    payload = await verifyMcpToken(token, kv);
  } catch (err) {
    if (err instanceof McpRevocationLookupError) {
      // ストア障害時はインフラ問題として 503 を返す。401 に潰すと正規トークンを誤って拒絶する。
      // A deny-list store outage must surface as 503, not 401, so legitimately signed tokens aren't misclassified.
...
export const mcpReadRequired = createMiddleware<AppEnv>(async (c, next) => {
  const payload = await extractAndVerify(c.req.header("Authorization"), c.get("kv"));
