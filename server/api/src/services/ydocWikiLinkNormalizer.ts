 ...
 * 純粋なヘルパー。クライアント側で同等処理を行っていた
 * `src/components/editor/extensions/applyWikiLinkMarksToEditor.ts` の責務を
 * サーバへ移管し、y-prosemirror の同期境界条件（lib0 `unexpectedCase`）が
 * 多数 mark 適用時にトリガされてしまう問題を恒久回避する（Issue #880 Phase B
 * リグレッション）。
 *
 * Pure helper that promotes unmarked `[[Title]]` plain text inside a Y.Doc to
 * `wikiLink` marks. Replaces the equivalent client-side helper
 * (`applyWikiLinkMarksToEditor`) so the boundary condition where a single
 * ProseMirror transaction carrying many `addMark` calls trips lib0's
 * `unexpectedCase` inside y-prosemirror is no longer reachable from the
 * synced document path (Issue #880 Phase B regression).
 *
 * 設計方針 / Design notes:
 ...
 * - Skip rules mirror the client implementation:
