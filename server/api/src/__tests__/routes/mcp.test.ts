...
describe("POST /api/mcp/authorize-code – additional edge cases", () => {
  beforeEach(() => {
    mockIsMcpRedirectUriAllowed.mockReturnValue(true);
  });

  it("returns 503 when redis context value is undefined", async () => {
    // Create an app where redis is not injected (undefined).
    const app = new Hono<AppEnv>();
    app.route("/api/mcp", mcpRoutes);
    vi.mocked(auth.api.getSession).mockResolvedValue({ user: mockSessionUser } as AuthSession);
    ...
    // 503 (Redis unavailable) or 401 (auth fails first without userId) — either is acceptable.
    expect([401, 503]).toContain(res.status);
  });

  it("uses default scopes (read+write) when scopes field is omitted", async () => {
    vi.mocked(auth.api.getSession).mockResolvedValue({ user: mockSessionUser } as AuthSession);
    const res = await createMcpApp(mockRedis, mockDb).request("/api/mcp/authorize-code", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        redirect_uri: "http://127.0.0.1:5173/cb",
        code_challenge: "default-ch",
      }),
    });
    expect(res.status).toBe(200);
    const body = (await res.json()) as { code?: string; state?: string };
    expect(typeof body.code).toBe("string");
    // storeMcpCode should have been called; the stored challenge embeds both scopes.
    expect(mockStoreMcpCode).toHaveBeenCalledOnce();
    const storedChallenge = mockStoreMcpCode.mock.calls[0]?.[3] as string;
    expect(storedChallenge).toContain("mcp:read");
    expect(storedChallenge).toContain("mcp:write");
  });

  it("returns empty string for state when state is not provided", async () => {
    vi.mocked(auth.api.getSession).mockResolvedValue({ user: mockSessionUser } as AuthSession);
    const res = await createMcpApp(mockRedis, mockDb).request("/api/mcp/authorize-code", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
describe("POST /api/mcp/session – additional edge cases", () => {
  ...
  it("scope embedded in challenge is parsed and used when minting token", async () => {
    // Simulate the route storing "challenge|mcp:read" (read-only scope).
    mockIsMcpRedirectUriAllowed.mockReturnValue(true);
    mockConsumeMcpCode.mockResolvedValue({
      userId: "user-scope-test",
      // Only mcp:read scope was stored
      codeChallenge: "mychallenge|mcp:read",
      redirectUri: "http://127.0.0.1:5173/cb",
    });
    mockVerifyPKCE.mockReturnValue(true);
    const res = await createMcpApp(mockRedis, mockDb).request("/api/mcp/session", {
    ...
    expect(res.status).toBe(200);
    expect(mockIssueMcpToken).toHaveBeenCalledWith("user-scope-test", ["mcp:read"]);
    const body = (await res.json()) as { scope?: string };
    expect(body.scope).toBe("mcp:read");
  });
});

describe("POST /api/mcp/clip – additional edge cases", () => {
  it("returns 400 when DNS check rejects the URL (SSRF protection)", async () => {
    const { isClipUrlAllowedAfterDns } = await import("../../lib/clipUrlPolicy.js");
    vi.mocked(isClipUrlAllowedAfterDns).mockResolvedValueOnce(false);
    ...
    const res = await createMcpApp(mockRedis, mockDb).request("/api/mcp/clip", {
      ...
      body: JSON.stringify({ url: "https://internal.corp/secret" }),
    });
    expect(res.status).toBe(400);
  });
});