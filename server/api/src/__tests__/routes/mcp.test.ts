...
describe("POST /api/mcp/authorize-code — additional edge cases", () => {
  ...
  it("returns 200 with empty state string when state is omitted", async () => {
    vi.mocked(auth.api.getSession).mockResolvedValue({ user: mockSessionUser } as AuthSession);
    const res = await createMcpApp(mockRedis, mockDb).request("/api/mcp/authorize-code", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  it("returns 200 when only mcp:read scope is requested", async () => {
    vi.mocked(auth.api.getSession).mockResolvedValue({ user: mockSessionUser } as AuthSession);
    const res = await createMcpApp(mockRedis, mockDb).request("/api/mcp/authorize-code", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  it("returns 400 when redirect_uri is empty string", async () => {
    vi.mocked(auth.api.getSession).mockResolvedValue({ user: mockSessionUser } as AuthSession);
    const res = await createMcpApp(mockRedis, mockDb).request("/api/mcp/authorize-code", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
describe("POST /api/mcp/session — additional edge cases", () => {
  it("includes scope field in successful response", async () => {
    mockIsMcpRedirectUriAllowed.mockReturnValue(true);
    mockConsumeMcpCode.mockResolvedValue({
      userId: "user-1",
      codeChallenge: "ch|mcp:read,mcp:write",
      redirectUri: "http://127.0.0.1:5173/cb",
    });
    mockVerifyPKCE.mockReturnValue(true);
    const res = await createMcpApp(mockRedis, mockDb).request("/api/mcp/session", {
    ...
    expect(body.scope).toMatch(/mcp:/);
  });

  it("returns 400 when redirect_uri is not in allowlist during session exchange", async () => {
    mockIsMcpRedirectUriAllowed.mockReturnValue(false);
    const res = await createMcpApp(mockRedis, mockDb).request("/api/mcp/session", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
describe("POST /api/mcp/clip — additional edge cases", () => {
  it("includes thumbnail_url in successful response", async () => {
    const res = await createMcpApp(mockRedis, mockDb).request("/api/mcp/clip", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
    ...
    expect(body.thumbnail_url).toBe("https://example.com/mcp-thumb.png");
  });

  it("returns 502 when clipAndCreate throws an error", async () => {
    const { clipAndCreate } = await import("../../lib/clipAndCreate.js");
    vi.mocked(clipAndCreate).mockRejectedValueOnce(new Error("clip service unavailable"));
    const res = await createMcpApp(mockRedis, mockDb).request("/api/mcp/clip", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
    ...
    const body = await parseJsonOrText(res);
    expect(body.message).toMatch(/clip|failed|try again/i);
  });

  it("returns 400 for private IP SSRF (192.168.x.x)", async () => {
    const res = await createMcpApp(mockRedis, mockDb).request("/api/mcp/clip", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: "Bearer t" },
