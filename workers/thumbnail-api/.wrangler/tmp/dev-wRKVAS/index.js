...
// .wrangler/tmp/bundle-ekX9aB/strip-cf-connecting-ip-header.js
...
var HonoRequest = /* @__PURE__ */ __name(class {
   ...
   */
  get matchedRoutes() {
    return this.#matchResult[0].map(([[, route3]]) => route3);
  }
  /**
   ...
   */
  get routePath() {
    return this.#matchResult[0].map(([[, route3]]) => route3)[this.routeIndex].path;
  }
}, "HonoRequest");
...
// src/services/search/openverse.ts
var DEFAULT_OPENVERSE_API = "https://api.openverse.org/v1/images/";
async function searchOpenverse(query, page, limit, apiBase = DEFAULT_OPENVERSE_API) {
  if (!query)
    return [];
  ...
  return results.map((result) => {
    if (!result.url || !result.thumbnail)
      return null;
    return {
      id: result.id,
      ...
      authorName: result.creator || void 0,
      authorUrl: result.creator_url || void 0
    };
  }).filter((item) => item !== null);
}
__name(searchOpenverse, "searchOpenverse");

// src/utils/normalize.ts
var stripHtml = /* @__PURE__ */ __name((value) => (value || "").replace(/<[^>]*>/g, "").replace(/\s+/g, " ").trim(), "stripHtml");

// src/services/search/wikipedia.ts
var DEFAULT_WIKIPEDIA_API = "https://ja.wikipedia.org/w/api.php";
var DEFAULT_WIKIPEDIA_REST = "https://ja.wikipedia.org/api/rest_v1";
async function searchWikipedia(query, apiBase = DEFAULT_WIKIPEDIA_API, restBase = DEFAULT_WIKIPEDIA_REST) {
  if (!query)
    return [];
  ...
  if (!previewUrl || !imageUrl)
    return [];
  const pageUrl = summary.content_urls?.desktop?.page || `${restBase.replace("/api/rest_v1", "")}/wiki/${encodeURIComponent(
    firstResult.title
  )}`;
  const altText = stripHtml(summary.extract) || summary.title || firstResult.title || query;
  return [
    {
...
// src/services/search/wikimedia.ts
var DEFAULT_WIKIMEDIA_API = "https://commons.wikimedia.org/w/api.php";
async function searchWikimedia(query, offset, limit, apiBase = DEFAULT_WIKIMEDIA_API) {
  if (!query)
    return [];
  const params = new URLSearchParams({
    action: "query",
  ...
  for (const page of pages) {
    ...
    const description = stripHtml(info.extmetadata?.ObjectName?.value) || stripHtml(info.extmetadata?.ImageDescription?.value);
...
__name(searchWikimedia, "searchWikimedia");
...
async function searchImages({
  ...
  const wikipediaItems = page === 1 ? await searchWikipedia(query, env.WIKIPEDIA_API_URL, env.WIKIPEDIA_REST_URL) : [];
  ...
  const [wikimedia, openverse] = await Promise.all([
    primaryLimit > 0 ? searchWikimedia(
      query,
      (page - 1) * primaryLimit,
      primaryLimit,
      env.WIKIMEDIA_API_URL
    ) : Promise.resolve([]),
    secondaryLimit > 0 ? searchOpenverse(
      query,
      page,
      secondaryLimit,
  ...
  const nextCursor = uniqueItems.length === 0 ? void 0 : String(page + 1);
  return { items: uniqueItems, nextCursor };
}
__name(searchImages, "searchImages");
...
route.get("/image-search", async (c) => {
  ...
  if (!query) {
    return c.json({ items: [], nextCursor: void 0 });
  }
  try {
    ...
    return c.json(
      {
        error: error instanceof Error ? error.message : "\u753B\u50CF\u691C\u7D22\u306B\u5931\u6557\u3057\u307E\u3057\u305F"
      },
      500
    );
  }
});
var image_search_default = route;

// src/services/gyazo.ts
var GYAZO_UPLOAD_URL = "https://upload.gyazo.com/api/upload";
var guessFileName = /* @__PURE__ */ __name((sourceUrl, fallback) => {
  try {
...
var fetchImage = /* @__PURE__ */ __name(async (url) => {
...
async function uploadToGyazo(sourceUrl, accessToken, title, fallbackUrl) {
...
__name(uploadToGyazo, "uploadToGyazo");

// src/routes/thumbnail-commit.ts
var route2 = new Hono2();
route2.post("/thumbnail/commit", async (c) => {
  let body = null;
  try {
  ...
  const headerToken = c.req.header("x-gyazo-access-token") || c.req.header("authorization")?.replace(/^Bearer\s+/i, "");
...
var thumbnail_commit_default = route2;
...
app.route("/api", image_search_default);
app.route("/api", thumbnail_commit_default);
var src_default = app;
...
// .wrangler/tmp/bundle-ekX9aB/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ekX9aB/middleware-loader.entry.ts
