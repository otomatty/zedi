...
```bash
bunx tsc --noEmit --pretty 2>&1 | tee /tmp/tsc_review.log; grep -F "$(echo '<変更ファイルパス>' | tr '\n' '|')" /tmp/tsc_review.log || true
```

> **変更ファイルフィルタ**: `tee` で全出力を `/tmp/tsc_review.log` に保存し、Phase 3 の分析では**変更ファイルに関連するエラーのみ**を対象とする（ファイル名で grep してフィルタ）。
